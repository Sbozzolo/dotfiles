...
* EXWM
  #+BEGIN_SRC emacs-lisp
    (use-package exwm
      ...
      ;; Launch multi-term
      (exwm-input-set-key (kbd "s-t") 'get-term)
      ...
      (exwm-input-set-key (kbd "<print>")
                          #'(lambda () (interactive)
                              (progn
                                ...
                                (message "Screenshot taken! Saved in ~/screenshots")
                                )
                              )
                          )
            ...
            (add-hook 'exwm-randr-screen-change-hook
                      (lambda ()
                        (start-process-shell-command
                         "xrandr" nil "xrandr --output DP-2 --same-as eDP-1 --auto")))
            (setq exwm-randr-clone t)
            (message "Output cloning, remove VGA cable, wait 1 s, insert VGA cable")
            (redraw-display)
            )
          )
          )
